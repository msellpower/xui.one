...
# --- רכיב UX: כפתור כלי עבודה (Tool Button) ---
...
class ProGauge(QWidget):
    def __init__(self, title, unit, max_val=100, color="#00d4ff"):
        super().__init__()
        self.value = 0; self.max_val = max_val; self.title = title; self.unit = unit; self.primary_color = QColor(color)
        self.setMinimumSize(180, 180)
    def set_value(self, val): self.value = val; self.update()
    def paintEvent(self, event):
        p = QPainter(self); p.setRenderHint(QPainter.RenderHint.Antialiasing)
        w, h = self.width(), self.height(); size = min(w, h) - 20; rect = QRectF((w-size)/2, (h-size)/2, size, size)
        p.setPen(QPen(QColor("#2d303e"), 12, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)); p.drawArc(rect, 135*16, 270*16)
        ratio = self.value/self.max_val; angle = int(270*ratio); angle = 270 if angle>270 else angle
        col = self.primary_color if ratio < 0.85 else QColor("#ff2e63")
        p.setPen(QPen(col, 12, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)); p.drawArc(rect, 225*16, -angle*16)
...
class StreamWorker(QObject):
    ...
    def __init__(self, name, url, config, record):
        super().__init__(); self.name=name; self.url=url; self.config=config; self.rec=record; self.running=True; self.proc=None
    ...
    def _get_xui(self):
        try:
            ...
            if not cat: cat=requests.post(f"{api}?action=add_category", data={**c,"category_name":"Channels","category_type":"live"}).json().get('category_id',"1")
            requests.post(f"{api}?action=add_stream", data={"username":c['user'],"password":c['pass'],"stream_display_name":self.name,"stream_source":["127.0.0.1"],"category_id":cat,"stream_mode":"live"})
    ...
    def run(self):
        ...
        xui = self._get_xui(); 
        try: requests.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage", data={"chat_id":TELEGRAM_CHAT_ID,"text":f"🟢 {self.name} Started"})
        except: pass
        ...
        while self.running:
            ...
            try:
                ...
                while self.proc.poll() is None and self.running:
                    ...
                    if self.rec: d_size=sum(os.path.getsize(os.path.join(folder,f)) for f in os.listdir(folder))/1048576
...
class XHotelUI(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("X-HOTEL MANAGER v12.0 (Ultimate Tools)"); self.resize(1600, 1000)
        self.workers={}; self.net_io=psutil.net_io_counters()
        self.setup_ui(); QTimer.singleShot(500, self.restore)
        self.t=QTimer(); self.t.timeout.connect(self.upd_stats); self.t.start(1000)
    ...
    def setup_ui(self):
        ...
        c_f=QFrame(); gl=QGridLayout(c_f); c_f.setStyleSheet("background:#1f2233;border-radius:12px;padding:10px;")
        self.url=QLineEdit("http://144.91.86.250/mbmWePBa"); self.usr=QLineEdit("admin"); self.pw=QLineEdit("MazalTovLanu"); self.m3u=QLineEdit()
        ...
        self.tbl=QTableWidget(0,7); self.tbl.setHorizontalHeaderLabels(["SEL","CHANNEL","REC","STATUS","UPTIME","DISK","ACTION"]); self.tbl.horizontalHeader().setSectionResizeMode(1)
        t1l.addWidget(self.tbl)
        
        acts=QHBoxLayout(); b1=QPushButton("START STREAMING"); b1.setStyleSheet("background:#00e676;color:black;font-weight:bold;padding:15px;"); b1.clicked.connect(self.start_sel)
        b2=QPushButton("STOP ALL"); b2.setStyleSheet("background:#ff2e63;color:white;font-weight:bold;padding:15px;"); b2.clicked.connect(self.stop_all)
        acts.addWidget(b1); acts.addWidget(b2); t1l.addLayout(acts)
        tabs.addTab(t1,"📡 OPERATIONS")

        # Tab 2: Analytics
        t2=QWidget(); t2l=QVBoxLayout(t2); gs=QHBoxLayout()
        self.g_cpu=ProGauge("CPU","%"); self.g_ram=ProGauge("RAM","%",color="#aa00ff"); self.g_dl=ProGauge("DL","MB",50,"#00e676"); self.g_ul=ProGauge("UL","MB",50,"#ffea00")
        gs.addWidget(self.g_cpu); gs.addWidget(self.g_ram); gs.addWidget(self.g_dl); gs.addWidget(self.g_ul); t2l.addLayout(gs)
        self.log=QTextEdit(); self.log.setReadOnly(True); t2l.addWidget(QLabel("LOGS:")); t2l.addWidget(self.log)
        tabs.addTab(t2,"📊 METRICS")

        # Tab 3: System Tools (NEW)
        t3=QWidget(); t3l=QGridLayout(t3); t3l.setSpacing(20)
        
        btn_open = ToolButton("OPEN RECORDINGS", "📂", "#3f51b5"); btn_open.clicked.connect(self.tool_open_folder)
        btn_clean = ToolButton("CLEAN DISK NOW", "🧹", "#ff9800"); btn_clean.clicked.connect(self.tool_clean_disk)
        btn_reboot = ToolButton("REBOOT SERVER", "🔄", "#d32f2f"); btn_reboot.clicked.connect(self.tool_reboot)
        btn_restart = ToolButton("RESTART APP", "🛑", "#00bcd4"); btn_restart.clicked.connect(self.tool_restart_app)
        
        t3l.addWidget(btn_open, 0, 0); t3l.addWidget(btn_clean, 0, 1)
        t3l.addWidget(btn_reboot, 1, 0); t3l.addWidget(btn_restart, 1, 1)
    ...
    def tool_open_folder(self):
        try:
            # מנסה לפתוח את התיקייה עם סייר הקבצים של המערכת
            subprocess.Popen(['xdg-open', RECORDINGS_PATH])
            self.add_log("Opening recordings folder...")
        except Exception as e:
            QMessageBox.warning(self, "Error", f"Cannot open folder: {e}\nTry installing 'nautilus' or 'pcmanfm'.")

    def tool_clean_disk(self):
        reply = QMessageBox.question(self, 'Clean Disk', "Delete all recordings older than 24h?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            os.system("/root/clean_recordings.sh")
            self.add_log("Disk cleanup command sent.")
            QMessageBox.information(self, "Success", "Cleanup script executed.")

    def tool_reboot(self):
        reply = QMessageBox.critical(self, 'REBOOT', "Are you sure you want to REBOOT the entire server?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            self.add_log("Rebooting server...")
            os.system("reboot")
    ...
    def upd_stats(self):
        ...
        n=psutil.net_io_counters(); self.g_dl.set_value((n.bytes_recv-self.net_io.bytes_recv)/1048576); self.g_ul.set_value((n.bytes_sent-self.net_io.bytes_sent)/1048576); self.net_io=n
    
    def load_m3u(self):
        try:
            d=requests.get(self.m3u.text(), timeout=10).text; self.tbl.setRowCount(0); self.db=[]
            n="Cam"; 
            for l in d.splitlines():
                if "#EXTINF" in l: n=l.split(",")[-1].strip()
                elif l.startswith("http"):
                    r=self.tbl.rowCount(); self.tbl.insertRow(r); self.db.append({"name":n,"url":l})
                    ...
                    b=QPushButton("STOP"); b.setStyleSheet("background:#2d303e;color:#ff2e63;"); b.clicked.connect(lambda _,x=n:self.stop_one(x)); self.tbl.setCellWidget(r,6,b)
        ...
        for r in range(self.tbl.rowCount()):
            if self.tbl.cellWidget(r,0).layout().itemAt(0).widget().isChecked():
                ...
                if n not in self.workers:
                    w=StreamWorker(n,self.db[r]['url'],cf,rec); w.stats_signal.connect(self.upd_row); self.workers[n]=w
    ...
    def upd_row(self,n,s):
        for r in range(self.tbl.rowCount()):
            if self.tbl.item(r,1).text()==n:
                self.tbl.item(r,3).setText(s['status']); self.tbl.item(r,3).setForeground(QColor("#00e676")); self.tbl.item(r,4).setText(s['uptime']); self.tbl.item(r,5).setText(s['disk'])
    ...
    def save(self):
        ...
        with open(CONFIG_FILE,"w") as f: json.dump({"url":self.url.text(),"user":self.usr.text(),"pass":self.pw.text(),"m3u":self.m3u.text(),"act":act},f)
    def restore(self):
        if os.path.exists(CONFIG_FILE):
            try: 
                with open(CONFIG_FILE) as f: s=json.load(f)
                self.url.setText(s.get("url","")); self.usr.setText(s.get("user","")); self.pw.setText(s.get("pass","")); self.m3u.setText(s.get("m3u",""))
                if s.get("m3u"):
